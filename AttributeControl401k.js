...
/*
  ...
  //example
    ...
    【注意】
    プルダウンの選択肢にlabelをつけたい場合はAttributeControl.js(戸倉さん開発)に直す必要があります。
...
class AttributeControl {
  ...
  setAttribute(autocompleteList_obj) {
    Object.keys(autocompleteList_obj).forEach((fieldcode) => {
      ...
      Object.keys(attributeList_obj).forEach((attribute_type) => {
        //各attributeに対して
        this.set(fieldcode, attribute_type, attributeList_obj[attribute_type]);
      });
    });
  }
  ...
  update_datalist(datalist_id, dataObject) {
    ...
    dataObject.forEach((obj) => {
      ...
      // ここからAttribute.jsとの修正点
      if (obj.code === "") newOption.label = ``;
      else newOption.label = ``;
      // ここまで修正点
